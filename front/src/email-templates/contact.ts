...
export const getContactEmailTemplate = ({ ...formInfo }: Props) => {
	const { username, email, message, select, lang } = formInfo;
	const user = username.charAt(0).toUpperCase() + username.substring(1);
    ...
    <body>
      <div class="container">
        ...
        <div class="contact-section">
          ...
          <p class="label"><strong>${t('email.template.email')}</strong> ${email}</p>

          ${select ? `<p class="label" > <strong>${t('email.template.asunto')}: </strong> ${select}</p > ` : ''}
        ...
        <div class="message-section">
          <p class="label"><strong>${t('email.template.mensaje')}</strong></p>
