...
export const getServiceClientEmailTemplate = ({ ...formInfo }: ClientProps) => {
	const { serviceUsername, lang, service } = formInfo;
	const user =
    ...
    <head>
      <meta charset="UTF-8" />
      <title> &#128075;  ${t('email.client.template.title')} ${serviceUsername}</title>
      <style>
    ...
    <body>
      <div class="container">
        ...
        <h1 class="heading">&#128075;  ${t('email.client.template.title')} ${user}</h1>
        <p class="text">${t('email.client.template.info')}</p>
        <p class="text">${t('email.client.template.saludo')}</p>
        <hr />
				 ${service ? `<p p class="label" > <strong>${t('email.template.service')}: </strong> ${service}</p > ` : ''}
        <div class="footer">
